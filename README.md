...
This variable will get pulled in during the netlify build, and we will reference it in step 5 using `process.env.FAUNADB_SERVER_SECRET`.
...
## 3) In the root folder, create a file named webpack.config.js, and file it with the following:  
...
## 4) Modify the start and build scripts in the package.json file to reference the webpack.config.js file you just created:  
...
## 5) Create a new file in the src folder called 'createDB.js', fill it with the following:  
...
## 6) Open your faunadb in the faunadb web UI to confirm that the Message class and the index have been created
